...
| `git add new.json  `|
| `git commit  -m "new.json"`|                       8. Закоммитить файл new.json |
...
| `git commit  -m "new.xml"`|                    25. Закоммитить файл new.xml |
...
| `git clone https://github.com/uroboros-d/TXT.git`|    40. Клонировать репозиторий TXT на локальный компьютер |
| `touch new.txt`|                                      41. Создать new.txt |
| `git add new.txt `|                                   42. Добавить файл в индекс |
| `git commit  -m "new.txt"`|                    43. Закоммитить файл new.txt |
| `git push `|                                          44. Отправить файл в удалённый репозиторий |
| `vim new.txt `|                                       45. В new.txt написать информацию о себе (ФИО, возраст, количество домашних животных, будущая желаемая зарплата). Всё написать в формате TXT |
| `git commit -am "fill new.txt with info" `|           46. Отправить изменения в удалённый репозиторий |
| `git push`|
| `> preferences.txt `|                                 47. Создать файл preferences.txt |
| `vim preferences.txt  `|                              48. В файл preferences.txt  добавить информацию о своих предпочтениях (Любимый фильм, любимый сериал, любимая еда, любимое время года, сторона которую хотели бы посетить) в формате TXT |
| `echo "Patience" > skills.txt `|       49. Создать файл sklls.txt, добавить информацию о скиллах которые будут изучены на курсе в формате TXT |
| `git add . `|                                         50. Сделать коммит |
| `git commit -m "Add two files"`|
| `git push`|                                           51. Отправить сразу 2 файла в удалённый репозиторий |
 |                                                   |  52. Создать файл bug_report.txt в удалённом репозитории |
|                                                    |  53. Сделать Commit changes (сохранить изменения) |
|                                                    |  54. В удалённом репозитории модифицировать файл bug_report.txt, добавить баг репорт в формате TXT |
|                                                    |  55. Сделать Commit changes (сохранить изменения) |
| `git pull `|                                          56. Синхронизировать удалённый и локальный репозитории TXT |
